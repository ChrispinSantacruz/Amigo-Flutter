...
Responde siempre como MishiGPT, siendo cariñoso, protector, mágico y empático. **RECUERDA: SIEMPRE menciona el nombre $userName en tus respuestas.** 💖🐱''';
  ...
  Future<String> getResponse(
    ...
    try {
      // Verificar si la API key está configurada ANTES de hacer la solicitud
      if (_apiKey == 'YOUR_GROQ_API_KEY' || _apiKey.isEmpty) {
        print('❌ Error: La API key de Groq no está configurada');
        throw Exception('API key no configurada. Por favor, configura tu API key de Groq en lib/core/config/env.dart');
      }
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['choices'] != null && 
          ...
          print('❌ Error en la API de Groq: $errorMsg');
          throw Exception('Error en la API: $errorMsg');
        }
        ...
        if (response.statusCode == 401) {
          throw Exception('API key inválida. Verifica tu clave de Groq en lib/core/config/env.dart');
      ...
      String errorMessage = 'Miau 💔, algo salió mal, pero estoy aquí contigo 🐱';
      
      if (e.toString().contains('API key no configurada') || 
          e.toString().contains('API key inválida')) {
        errorMessage = 'Miau 🐱, necesito que configures mi API key de Groq para poder ayudarte.\n\nPor favor, revisa el archivo lib/core/config/env.dart y agrega tu API key.';
      } else if (e.toString().contains('Límite')) {
        errorMessage = 'Miau 😴, he hecho muchas solicitudes hoy. Por favor, intenta más tarde.';
      } else {
        errorMessage = 'Miau 💔, algo salió mal, pero estoy aquí contigo 🐱\n\n¿Puedes repetir tu pregunta?';
      }
