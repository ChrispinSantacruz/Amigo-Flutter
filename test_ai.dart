// Script de prueba para verificar la conexión con Hugging Face y el modelo de IA
// Ejecuta este archivo con: dart test_ai.dart

import 'dart:convert';
import 'package:http/http.dart' as http;

class AIServiceTest {
  final String _apiUrl = 'https://api-inference.huggingface.co/models/meta-llama/Llama-3.1-8B-Instruct';
  final String _token = 'YOUR_HUGGING_FACE_TOKEN'; // Replace with your actual token

  // Prompt del sistema con la personalidad del gatito
...
Responde siempre como el gatito mágico Amigo, siendo cariñoso, protector y mágico, pero serio cuando se trata de problemas reales.''';
  ...
  Future<String> getResponse(String userMessage, String userName, int userAge) async {
    try {
      ...
      print('\n⏳ Esperando respuesta del modelo (puede tardar 10-30 segundos en la primera llamada)...\n');
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData is List && responseData.isNotEmpty) {
          ...
          print('\n📄 Texto generado (sin procesar):\n$generatedText\n');
          
          // Limpiar la respuesta y asegurar que empiece con "¡Miau!"
        ...
        throw Exception('Respuesta inesperada de la API: $responseData');
...
void main() async {
  ...
  // Pruebas con diferentes mensajes
  final testCases = [
    {
  ...
  for (int i = 0; i < testCases.length; i++) {
    ...
    try {
      ...
      // Verificar que empiece con "¡Miau!"
      if (response.startsWith('¡Miau') || 
          response.startsWith('¡Ronroneo') || 
          response.startsWith('¡Purr')) {
        print('\n✅ ✓ La respuesta comienza correctamente con el saludo del gatito');
      } else {
        print('\n⚠️  La respuesta NO comienza con el saludo esperado');
      }
    ...
    if (i < testCases.length - 1) {
      print('\n⏸️  Esperando 3 segundos antes de la siguiente prueba...');
